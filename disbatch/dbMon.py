        ...
        if j != b'<Starting...>':
            ...
            header.append(
                (
                    [Vertical]
                    + [
                        'Slots {slots:5d}                  Tasks: Finished {finished:7d}      Failed{failed:5d}      Barrier{barriers:3d}'.format(
                            **statusd
                        )
                    ]
                    + [Vertical],
            ...
            header.append(
                (
                    [Vertical]
                    + ['Rank    Context           Host          Last     Avail   Assigned   Finished   Failed']
                    + [Vertical],
                    CPCB,
                )
            )
            ...
            for rank, engine in ee:
                ...
                content.append(
                    (
                        rank,
                        '{rank:5d} {cLabel:12.12s} {hostname:20.20s} {delay:6.0f}s {active:7d} {assigned:10d} {finished:10d} {failed:7d}'.format(
                            **engine
                        ),
                    )
                )
...
def popYNC(msg, parent, inq, title='Confirm'):
    ...
    h = int(ph * 0.75)
    w = int(pw * 0.85)
    ro, co = int((ph - h) * 0.5), int((pw - w) * 0.5)
    ...
    nw.border()
    nw.addstr(0, int((w - len(title)) * 0.5), title)
    for r, L in enumerate(msgw):
        nw.addstr(r + 1, 1, L)
    nw.addstr(r + 2, int((w - 19) * 0.5), '[Y]es/[N]o/[C]ancel', curses.A_REVERSE)
    nw.refresh()
                ...
                if not done:
                    ...
                    if target is not None:
                        if k == ord('C'):
                            ...
                            r = popYNC(
                                'Stopping engine {rank:d} ({hostname:s}, {pid:d})'.format(**engines[target]), S, inq
                            )
                            if r == 'Y':
                                try:
