...
def addKVSServerArgument(argp, name='kvsserver'):
    ...
    argp.add_argument(
        name,
        metavar='host:port',
        nargs='?' if port else None,
        default=host + ':' + port if port else None,
...
if '__main__' == __name__:
    ...
    argp.add_argument(
        '-R', '--retry', default=0, type=int, metavar='COUNT', help='Number of times to retry on connect failure [0]'
    )
    argp.add_argument(
        '-P', '--pickle', dest='encoding', action='store_true', help='(Un-)Pickle values to/from python expressions'
    )
    argp.add_argument(
        '-A', '--no-pickle', dest='encoding', action='store_false', help="Don't (un-)pickle values (default)"
    )
    argp.add_argument(
        '-E',
        '--encoding',
        ...
        type=str,
        metavar='CODE',
        help='Explicitly set/get encoding (4-character string, ignored on get) [ASTR or PYPK with -P]',
    )
    ...
    argp.add_argument(
        ...
        action=OpAction,
        nargs=2,
        metavar=('MKEY', 'KEY:EVENTS'),
        help='Create or update a monitor for the key and events',
    )
