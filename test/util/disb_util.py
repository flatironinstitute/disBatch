...
def hint_msg (state):
    if state == NEED_TASKFILE:
        return "Please input your taskfile: "
    return "$ "

def help_msg (state):
    if state == NEED_TASKFILE:
        print('Pease import an taskfile. File with tasks, one task per line ("-" for stdin)')

def exec_cmd (curr_state, cmd):
    if state == NEED_TASKFILE:
        

def shell():
    curr_state = NEED_TASKFILE
    while True:
        hint    = hint_msg(curr_state)
        command = input(hint)
        if command == "exit":
            break
        elif command == "help":
...
if '__main__' == __name__:
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--shell':
        ...
        os.system("/cm/shared/sw/pkg/flatiron/disBatch/2.0-beta/disBatch {}".format(sys.argv[1:]))
